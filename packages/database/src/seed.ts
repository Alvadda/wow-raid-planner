...
const createFactions = async () =>
  await Promise.all(
    factions.map((f) => prisma.faction.create({ data: { name: f } }))
  );
...
const createServerAndRegion = async () => {
  ...
  await Promise.all(
    servers.map((s) => {
      ...
      if (!regionId)
        throw new Error(`no region found for Server ${s.server} - ${s.region}`);
...
const createSpecs = async (
  rolesDb: Map<string, string>,
  classesDb: Map<string, string>
) => {
  const specs = new Set<{ name: string; class: string; role: string }>();
  charData.forEach((cd) =>
    cd.specs.forEach((s) =>
      specs.add({ name: s.name, class: cd.class, role: s.role })
    )
  );
  ...
  for (const spec of specs.values()) {
    ...
    if (!roleId || !classId)
      throw new Error(`Role or Class does not exit for spec: ${spec}`);
